...
export const startWalk = async (req, res) => {
    try {
        ...
        const job = await safetyQueue.add("safety-check", { sessionId: session.id, userId }, { delay });
...
export const getWalkHistory = async (req, res) => {
    try {
        const userId = req.user.id;
...
export const getPublicSession = async (req, res) => {
    try {
        ...
        if (!session)
            return res.status(404).json({ message: "Alert session not found" });
        res.json(session);
    }
    catch (error) {
        res.status(500).json({ error: "Internal server error" });
