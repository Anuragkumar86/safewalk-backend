...
export const safetyWorker = new Worker("safety-timer", async (job) => {
    ...
    if (session && session.status === "ACTIVE") {
        ...
        for (const contact of session.user.contacts) {
            ...
            try {
                await sendEmergencyEmail(contact.email, session.user.name, session.lastKnownLat, session.lastKnownLng, sessionId // Pass the ID for the custom tracking link
                );
            }
            catch (err) {
...
//# sourceMappingURL=safetyWorker.js.map